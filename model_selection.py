...
def random_search(X_train, Y_train, X_val, Y_val, cv):
    """
    Apply random search.
    cv is the number of folds for k-fold cross validation.
    """
    if cv == 1 and X_val is None:
        raise ValueError("If cv=1 you must pass X_val and Y_val with not None values (i.e apply classic split of Training Set)...")
...
# random
random_search(X_train, Y_train, None, None, cv=5)
